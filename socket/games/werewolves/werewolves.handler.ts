...
const WerewolvesHandler = (io: IoType, socket: SocketType) => {
  ...
  socket.on("game:werewolves:witch:save", ({ roomId, userId: playerId, target }: IWerewolvesSendTarget) => onWitchSavePlayer(io, socket)({ roomId, userId: playerId, target }));
  socket.on("game:werewolves:witch:kill", ({ roomId, userId: playerId, target }: IWerewolvesSendTarget) => onWitchKillPlayer(io, socket)({ roomId, userId: playerId, target }));
  socket.on("game:werewolves:witch:skip", ({ roomId, playerId }: { roomId: string, playerId: string }) => onWitchSkip(io, socket)({ roomId, playerId }));
  socket.on("game:werewolves:hunter:kill", ({ roomId, userId: playerId, target } : IWerewolvesSendTarget) => onHunterKillPlayer(io, socket)({ roomId, userId: playerId, target }));
